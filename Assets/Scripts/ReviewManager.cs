...
public class ReviewManager : MonoBehaviour
{
    ...
    public static ReviewManager Instance
    {
        get
        {
            if (_instance == null)
            {
                var obj = new GameObject("ReviewManager");
                _instance = obj.AddComponent<ReviewManager>();
            }
            return _instance;
        }
    }
    ...
    public void SaveReview(string locationId, string remarks, string userName, int rating, Action<bool> onComplete = null)
    {
        ...
        try
        {
            ...
            Debug.Log($"[ReviewManager.SaveReview] Generated review ID: {reviewId}");
            ...
            Debug.Log($"[ReviewManager.SaveReview] Full path: /{locationId}/Reviews/{reviewId}");
            Debug.Log($"[ReviewManager.SaveReview] Payload: Remarks='{remarks}', UserName='{userName}', Rating={rating}");
            Debug.Log($"[ReviewManager.SaveReview] Calling SetValueAsync...");

            var task = newReviewRef.SetValueAsync(reviewData);
            Debug.Log($"[ReviewManager.SaveReview] SetValueAsync task created");

            task.ContinueWithOnMainThread(t =>
            {
                Debug.Log($"[ReviewManager.SaveReview] Task completed. IsFaulted={t.IsFaulted}, IsCanceled={t.IsCanceled}, IsCompleted={t.IsCompleted}");

                if (t.IsFaulted)
                {
                    Debug.LogError($"[ReviewManager.SaveReview] FAULTED: {t.Exception?.Message}");
                    if (t.Exception != null)
                    {
                        foreach (var ex in t.Exception.InnerExceptions)
                        {
                            Debug.LogError($"[ReviewManager.SaveReview] Inner exception: {ex.GetType().Name}: {ex.Message}");
                        }
                        // Print full stack trace for debugging
                        Debug.LogError($"[ReviewManager.SaveReview] Full exception:\n{t.Exception}");
                    }
                    onComplete?.Invoke(false);
                    return;
                }
                ...
                if (t.IsCanceled)
                {
                    Debug.LogError($"[ReviewManager.SaveReview] CANCELED");
                    onComplete?.Invoke(false);
                    return;
                }

                Debug.Log($"[ReviewManager.SaveReview] SUCCESS! Review saved with ID: {reviewId}");
                onComplete?.Invoke(true);
            });
        }
        catch (Exception ex)
        {
            Debug.LogError($"[ReviewManager.SaveReview] EXCEPTION: {ex.Message}\n{ex.StackTrace}");
            onComplete?.Invoke(false);
        }
    }
