...
public class SignUpSceneScript : MonoBehaviour
{
    ...
    private async Task SignUpUser(string email, string password)
    {
        ...
        try
        {
            var userCredential = await auth.CreateUserWithEmailAndPasswordAsync(email, password).ConfigureAwait(false);
            Debug.LogFormat("Firebase user created successfully: {0} ({1})",
                userCredential.User.DisplayName, userCredential.User.UserId);
        }
